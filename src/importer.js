// Functions for dependencies import
...
class Importer {
  ...
  import() {
    ...
    // Don't rewrite imports if files weren't changed
    if (!(this.prevFiles && _.isEqual(this.files, this.prevFiles))) {
      this.addDependencies();
    }

    // Rewrite imports if new files were added (if this.create is true)
    if (this.repeat) {
      this.import();
    }
  }
  ...
  injectImports(itemFile, neededImports) {
    ...
    if (!neededImports.length) {
      ...
      if (mainContent.match(/\S/) || !(this.clear && blockContent)) {
        // File has some content beside import block or Importer is setted to
        // not clear files or file wasn't created by Importer (no block content).
